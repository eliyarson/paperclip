...
export const jsonSchemaSchema = z.record(z.unknown()).refine(
  (val) => {
    ...
    // Reject arrays and primitives at the top level
    if (Array.isArray(val) || typeof val !== "object" || val === null) {
      return false;
    }
    // Must have at least one JSON Schema discriminator
    const hasDiscriminator =
      typeof val.type === "string" && val.type.length > 0
      || typeof val.$ref === "string" && val.$ref.length > 0
      || Array.isArray(val.oneOf) && val.oneOf.length > 0
      || Array.isArray(val.anyOf) && val.anyOf.length > 0
      || Array.isArray(val.allOf) && val.allOf.length > 0
      || typeof val.const !== "undefined"
      || Array.isArray(val.enum) && val.enum.length > 0;
    return hasDiscriminator;
  },
  { message: "Must be a valid JSON Schema object (requires at least a non-empty 'type', '$ref', 'const', 'enum', or composition keyword 'oneOf'/'anyOf'/'allOf')" },
);
