...
- **Forge-linked issues (spec harness)**: When work follows the spec harness (spec → plan → build → eval), create issues with `originKind: "forge_charter"` and `originId: "<change-id>"`. These issues cannot be closed until the Forge Charter reaches `verified` or `archived`. If you get a **409** on close, check the Charter status and ensure eval gates pass before retrying. Use `POST /api/companies/:id/forge/observations` to write durable evidence back to the Charter.

## Memory Contract (Cerebro)

Paperclip uses Cerebro for long-term memory. Your AGENTS.md contains the full contract; key points:

- **Fetch context before work** when the issue involves prior decisions, project history, blockers, or continuity needs
- **Write observations before exit** when you create durable decisions, blockers, playbooks, outcomes, or evidence summaries
- **Use MCP tools directly** when available (`cerebro_context_search`, `cerebro_recall`, `cerebro_remember`)
- **Use Paperclip proxy fallback** when MCP is unavailable: `POST /api/agents/me/cerebro/context` and `POST /api/agents/me/cerebro/observations`
- **Mention degraded Cerebro** in your issue comment when memory capture was expected but unavailable
- **Never include secrets** in observation content (validated and rejected by proxy)
...
| Link issue to Forge Charter           | `POST /api/issues/:issueId/forge-link`                                                                                          |
| Get Forge link status                 | `GET /api/issues/:issueId/forge-link`                                                                                           |
| Resolve issue by Charter              | `GET /api/companies/:companyId/forge/charters/:changeId/issue`                                                                  |
| Write Forge observations              | `POST /api/companies/:companyId/forge/observations`                                                                             |
