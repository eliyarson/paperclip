...
export const ACTIVE_RUN_OUTPUT_CONTINUE_REARM_MS = 10 * 60 * 1000;
const ACTIVE_RUN_OUTPUT_EVIDENCE_TAIL_BYTES = 8 * 1024;
const NEEDS_FOLLOWUP_SUPERVISOR_REVIEW_ORIGIN_KIND = RECOVERY_ORIGIN_KINDS.needsFollowupSupervisorReview;
...
export function recoveryService(db: Db, deps: { enqueueWakeup: RecoveryWakeup }) {
  ...
  async function findOpenNeedsFollowupSupervisorReviewIssue(companyId: string, sourceIssueId: string) {
    return db
      .select()
      .from(issues)
      .where(
        and(
          eq(issues.companyId, companyId),
          eq(issues.originKind, NEEDS_FOLLOWUP_SUPERVISOR_REVIEW_ORIGIN_KIND),
          eq(issues.originId, sourceIssueId),
          isNull(issues.hiddenAt),
          notInArray(issues.status, ["done", "cancelled"]),
        ),
  ...
  async function resolveNeedsFollowupSupervisorAgentId(issue: typeof issues.$inferSelect) {
    const candidateIds: string[] = [];
    if (issue.assigneeAgentId) {
      const assignee = await getAgent(issue.assigneeAgentId);
      if (assignee?.reportsTo) candidateIds.push(assignee.reportsTo);
  ...
  function buildNeedsFollowupSupervisorReviewDescription(input: {
    ...
    return [
      "Paperclip detected assigned work whose latest run succeeded but ended in `needs_followup` without durable action evidence.",
      "",
      "## Source",
      "",
      `- Source issue: ${sourceIssue}`,
      `- Latest run: ${runLink}`,
      `- Latest liveness: \`${input.latestRun?.livenessState ?? "unknown"}\``,
      "",
      "## Required Action",
      "",
      "- Verify the partial work and decide whether it is acceptable, needs fixes, or should be reassigned.",
      "- If the original assignee should continue, leave a concise instruction on the source issue and mark this review issue done.",
      "- If you can finish the decision yourself, update the source issue accordingly, then mark this review issue done.",
      "- Do not leave the source issue without a concrete next action.",
    ].join("\n");
  }

  async function ensureNeedsFollowupSupervisorReviewIssue(input: {
    issue: typeof issues.$inferSelect;
    latestRun: LatestIssueRun;
  }) {
    if (input.issue.originKind === NEEDS_FOLLOWUP_SUPERVISOR_REVIEW_ORIGIN_KIND) return null;

    const existing = await findOpenNeedsFollowupSupervisorReviewIssue(input.issue.companyId, input.issue.id);
    if (existing) return { issue: existing, created: false };

    const ownerAgentId = await resolveNeedsFollowupSupervisorAgentId(input.issue);
    if (!ownerAgentId) return null;

    const prefix = await getCompanyIssuePrefix(input.issue.companyId);
    const review = await issuesSvc.create(input.issue.companyId, {
      title: `Review stalled follow-up for ${input.issue.identifier ?? input.issue.title}`,
      description: buildNeedsFollowupSupervisorReviewDescription({
        issue: input.issue,
      ...
      assigneeAgentId: ownerAgentId,
      originKind: NEEDS_FOLLOWUP_SUPERVISOR_REVIEW_ORIGIN_KIND,
      originId: input.issue.id,
      originRunId: input.latestRun?.id ?? null,
      originFingerprint: [
        NEEDS_FOLLOWUP_SUPERVISOR_REVIEW_ORIGIN_KIND,
    ...
    await deps.enqueueWakeup(ownerAgentId, {
      ...
      contextSnapshot: {
        ...
        taskId: review.id,
        wakeReason: "issue_assigned",
        source: NEEDS_FOLLOWUP_SUPERVISOR_REVIEW_ORIGIN_KIND,
        sourceIssueId: input.issue.id,
        sourceRunId: input.latestRun?.id ?? null,
      },
    });

    return { issue: review, created: true };
  }

  async function routeNeedsFollowupToSupervisor(input: {
    issue: typeof issues.$inferSelect;
    latestRun: LatestIssueRun;
  }) {
    const review = await ensureNeedsFollowupSupervisorReviewIssue(input);
    if (!review) return null;

    const blockerIds = await existingUnresolvedBlockerIssueIds(input.issue.companyId, input.issue.id);
    const nextBlockerIds = [...new Set([...blockerIds, review.issue.id])];
    ...
    if (review.created) {
      ...
      await issuesSvc.addComment(
        input.issue.id,
        [
          "Paperclip detected that the latest run ended with useful output but no durable conclusion.",
          "",
          `- Latest run: ${input.latestRun ? runUiLink({ id: input.latestRun.id, agentId: input.latestRun.agentId }, prefix) : "none"}`,
          `- Supervisor review: ${issueUiLink({ identifier: review.issue.identifier, id: review.issue.id }, prefix)}`,
          "- The source issue is blocked on that review so dependency wakeups can resume it after the supervisor decides the next action.",
        ].join("\n"),
        {},
      );
    }
    ...
    await logActivity(db, {
      ...
      details: {
        ...
        source: "recovery.needs_followup_supervisor_review",
        latestRunId: input.latestRun?.id ?? null,
        reviewIssueId: review.issue.id,
        reviewIssueCreated: review.created,
        blockerIssueIds: nextBlockerIds,
      },
    });
  ...
  async function reconcileStrandedAssignedIssues() {
    ...
    const result = {
      ...
      needsFollowupSupervisorReviews: 0,
    ...
    for (const issue of candidates) {
      ...
      if (isSuccessfulInProgressContinuationRun(latestRun)) {
        if (latestRun?.livenessState === "needs_followup") {
          const reviewIssue = await routeNeedsFollowupToSupervisor({ issue, latestRun });
          if (reviewIssue) {
            result.needsFollowupSupervisorReviews += 1;
            result.issueIds.push(issue.id);
          } else {
            result.skipped += 1;
          }
